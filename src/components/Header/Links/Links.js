		...
		<StyledLinks className="md:flex gap-8 capitalize font-semibold hidden">
			{links.map((link, i) => (
				...
				>
						...
						<div className="bg-white  absolute top-20 text-black overflow-hidden  rounded-lg grid grid-cols-2 lg:grid-cols-4 min-w-max opacity-90" id="links">
							{sublinks.map((link, i) => (
								<ul key={i} className="  pb-4">
									<h4 className="px-4 pt-4 mb-2 text-xs uppercase text-gray-700">
										{link.name}
									</h4>
									{link.links.map((link, i) => (
										<li
											key={i}
											className="px-4 py-2 text-sm cursor-pointer opacity-80 hover:opacity-100 hover:bg-gray-200"
										>
											{link}
										</li>
									))}
					...
					i === 2 && (
						<div className="bg-white  fixed left-1/2 transform -translate-x-1/2 max-w-3xl  mx-auto top-20 text-black overflow-hidden  rounded-lg grid grid-cols-2 xl:grid-cols-5 min-w-max opacity-90" id="news">
								...
								<ul key={i} className="  pb-4">
									...
									{link.links.map((link, i) => (
										<li
											...
											className="px-4 py-2 text-sm cursor-pointer opacity-80 hover:opacity-100 hover:bg-gray-200"
										>
											<img src={link} className="rounded" alt="" />
										</li>
									))}
								</ul>
							))}
